...
import { createOllamaInstance } from "../services/ollamaService";
import { createContainerByEngine } from "../services/engineRegistry";

export const dockerRoutes = new Elysia({ prefix: "/docker" }).post(
  "/create",
  async ({ body, set }) => {
    try {
      const result = await createOllamaInstance(body.modelName);
      set.status = 201;
      return {
        success: true,
        data: result,
