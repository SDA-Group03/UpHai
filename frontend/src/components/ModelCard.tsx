...
export const ModelCard = ({ data, onClick }: ModelCardProps) => {
  return (
    <Card
      onClick={onClick}
      className="group relative flex flex-col justify-between border-slate-100 shadow-sm hover:shadow-xl hover:border-purple-500/30 transition-all duration-300 cursor-pointer h-full hover:-translate-y-1 overflow-hidden"
    >
      {/* Header */}
      ...
      <CardContent className="p-5 pt-2 flex-1">
        <p className="text-sm text-slate-500 line-clamp-3 leading-relaxed">{data.description}</p>
      </CardContent>
      ...
      <CardFooter className="p-5 pt-0 flex flex-wrap gap-2 mt-auto">
        ...
        {data.performanceTier && (
          <Badge
            variant="outline"
            className="bg-slate-50 text-slate-600 border-slate-100 group-hover:bg-[#6E29F6]/5 group-hover:text-[#6E29F6] group-hover:border-[#6E29F6]/20 transition-colors"
          >
            {data.performanceTier}
          </Badge>
        )}
