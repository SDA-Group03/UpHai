...
const FilterGroup = ({ value, title, options }: FilterGroupProps) => {
  return (
    <AccordionItem value={value} className="border-b-0 mb-4">
      <AccordionTrigger className="py-2 hover:no-underline text-xs text-slate-500 font-bold uppercase tracking-wide hover:text-slate-700">{title}</AccordionTrigger>
      <AccordionContent>
  ...
  return (
    <ScrollArea className="h-full pr-4 pb-20 w-full">
      <Accordion type="multiple" defaultValue={['category', 'tags', 'series', 'context', 'size']} className="w-full">
        <FilterGroup value="category" title="Category" options={['Chat', 'Image', 'Video', 'Speech', 'Embedding', 'Reranker']} />
        <FilterGroup value="tags" title="Tags" options={['VLM', 'MoE', 'Tools', 'FIM', 'Math', 'Coder']} />
        <FilterGroup value="series" title="Series" options={['DeepSeek', 'Qwen', 'Llama', 'FLUX', 'MiniMax', 'Mistral']} />
